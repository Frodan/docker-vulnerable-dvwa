pipeline {
  ...
  stages {
    ...
    stage('Dependency Scan'){
     agent any
     steps{
       sh 'docker run -v "/home/frodan/jenkins/workspace/Thesis_master@2/package-lock.json:/package-lock.json" node:17-slim sleep 10'
     }
    }
    stage('SAST'){
      agent any
    ...
    stage('DAST') {
      ...
      steps{
      sh 'sleep 10'
      withCredentials([usernamePassword(credentialsId: 'DASTCredentials', passwordVariable: 'DASTpassword', usernameVariable: 'DASTlogin')]) {
      sh '''
